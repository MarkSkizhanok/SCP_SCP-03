...
module.exports = function () {
    ...
    this.on("CREATE", "CoffeeMachine", async (CoffeeMachine) => {
        ...
        if (!data.CMID) {
            data.CMID = await db.getNextval("cmid");
//		throw new Error(`Invalid email for ${data.FIRSTNAME}. No Way! E-Mail must be valid and ${data.EMAIL} has problems`);
    ...
    this.after("READ", "CoffeeMachine", (entity) => {
        if (entity.length > 0) {
            // entity.forEach(item => item.mandt = "");
            entity.forEach(item => item.date = "", item.numSimultCups = 0, item.descr = "");
        }
    });
